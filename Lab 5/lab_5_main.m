...
% The exact solution is y = (e^(-t/2))*(c1*cos(2t)+c2*sin(2t))
% The exact solution shows that it is a "decay while oscillating" function
% since the power of e is negative which makes the function decay and the
% cosine and sine part of the function makes the function oscillate
...
% (c)
% The exact solution:
% y = c1*e^(((2-sqrt(3))/2)*t)+c2*e^(((-2-sqrt(3))/2)*t)
% The graph of the function is not oscillating which is proven by the exact
% solution not having a cosine or sine component. 
% Also, positive exponent exponentials approaches infinity while negative
% exponent exponentials approaches 0 as t approaches positive infinity. 
% Therefore, the c2*e^(((-2-sqrt(3))/2)*t) term appraoches 0 as t 
% approaches positive infinity and the c1*e^(((2-sqrt(3))/2)*t) term
% becomes the dominant term. As a result, we would expect the function to
% grow.
...
% (c)
% The exact solution:
% y = c1*e^(((sqrt(2)-sqrt(3))/2)*t)+c2*e^(((-sqrt(2)-sqrt(3))/2)*t)
% The graph of the function is not oscillating which is proven by the exact
% solution not having a cosine or sine component. 
% Also, as previously mentioned, negative exponent exponentials approaches
% 0 as t approaches positive infinity. For this function, both terms have
% negative exponent exponentials. Therefore, the function is a decaying
% one.
...
%
% y = e^(-t)*(c1*sin(2*t)+c2*cos(2*t))+c3*sin(t)+c4*cos(t)
%
...
% Explaination:
% The function will basically oscillate since c3*sin(t) and c4*cos(t) will
% oscillate without approaching 0 while t approaches positive infinity.
% There is one exception case among the infinite cases, which is when both
% cos(t) and sin(t) are 0, then the function will decay while oscillating 
% due to the e^(-t) term in e^(-t)*(c1*sin(2*t)+c2*cos(2*t)).
