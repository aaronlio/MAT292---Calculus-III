...
% logical values."
%
% Error for zero index: "Array indices must be positive integers or
% logical values."
%
% Error for positive index outside of the bounds: "Index exceeds array
% bounds."
%
...
% Creation of vector x of 101 points
n = 101;
h = (7-(-3))/(n-1);
x = zeros(n,1);

% Preallocate the array y
y = zeros(n,1);

% Using a loop to equally space out points in vector x between -3 and 7
% and creating the function |y(i) = abs(x(i))*x(i)|
for i = 1:n
    x(i) = (-3)+(i-1)*h;
    y(i) = abs(x(i))*x(i);
end
...
% Creation of vector x of 5000001 points and equally spacing out the points 
% between -2 and 3
N = 5000001;
h = (3-(-2))/(N-1);
x = zeros(N,1);
...
% Clear out the array y
clear y;

% Begin timing the construciton and filling of the array by a loop
tic;
y = zeros(N,1);
for i = 1:N
    y(i) = atan(x(i));
...
% Begin timing the construciton and filling of the array by vectorization
tic;
y = atan(x);
% Stop the timing
toc;
...
% General solution:
% y = (c+t)/(exp(t^2))
% Particular solution with initial conditions y(1) = -1
% y = (-exp(1)-1+t)/(exp(t^2))
% Particular solution with initial conditions y(1) = 0
% y = (-1+t)/(exp(t^2))
% Particular solution with initial conditions y(1) = 1
% y = (exp(1)-1+t)/(exp(t^2))
...
% Particular solutions for the three specified initial conditions
Sol_1 = (-exp(1)-1+t)./(exp(t.^2));
Sol_2 = (-1+t)./(exp(t.^2));
Sol_3 = (exp(1)-1+t)./(exp(t.^2));

% Plot the graph
plot(t,Sol_1,t,Sol_2,t,Sol_3);
...
% Defining an inline function of one variable (Function Definition)
f = @(x) (x.^3 - abs(x)) .* log(x.^2+1);

% Computing and printing |f(0)| where x = 0
fprintf(' f(%g) = %g\n', 0, f(0));

% Computing and printing |f(1)| where x = 1
fprintf(' f(%g) = %g\n', 1, f(1));
...
% Label the graph
title('Graph of f(x) = (x.^3 - abs(x)) .* log(x.^2+1))')
xlabel('x');
ylabel('f(x)');
...
% Results for certain values of x and y (printing them out)
x = 0; y = 0;
fprintf(' f(x = %g, y = %g) = %g\n', x, y, f(x, y));
x = (pi/2); y = 1;
fprintf(' f(x = %g, y = %g) = %g\n', x, y, f(x, y));
...
%The first one of the two positive solutions at |y = 1/2|
y0 = (1/2);
guess = 10;
g = @(x) f(x,y0);
x = fzero(g, guess);
...
%The second one of the two positive solutions at |y = 1/2|
y0 = (1/2);
guess = 0.1;
g = @(x) f(x,y0);
x = fzero(g, guess);
...
% Function defintion
% NOTE: added 1 to the function since the original function does not pass
% through (0,0)
...
xlabel('x');
ylabel('y');
title('Solution to f(x,y) = y + y^3 - cos x + x^2 - x^4')
...
% The particular solution with y(0) = 1
% Since c = 0
% --> x^3-sin(x)-ln|y| = 0

% Function definition
f = @(x,y) x.^3-sin(x)-log(abs(y));
...
xlabel('x');
ylabel('y');
title('Solution to dy/dx = (-cos x + 3x^2) y')
...
% The behaviour of the solutions are that:
% 1. Since there are two asymptotes (the place for the asymptote depends 
% on what the constant c is) at each side of y (the slope function is an 
% even function, so it is symmetric about the y axis), y approches 
% infinity at there.
% 2. As x appraoches infinty, y approaches 0.
% 3. The slope function is symmetric about the y-axis. 
% 4. The part of the graph above the x-axis looks like a parabola with
% asymptotes on both sides
% 5. The part of the graph below the x-axis looks like a -1/(x^2) function

% As x appraoches infinty, y approaches 0.

% Solutions are symmetric about the y-axis because the function is an even
% function.
% The sign of the slope only depends on the sign of x (since it is 
% y^2); therefore, for any given pair of x and y, you can plug in a 
% positive and a negative x, which would give you a positive and negative 
% slope respectively. In other words, for every point, there is a 
% reflected point with a corresponding negative slope. Therefore, the 
% function is reflected about the y-axis. 
...
% For y_0 = 2: 
% y approaches 2, as x approaches both positive and negative infinity.

% For 1 < y_0 < 2:
% As x approaches positive infinity, y approaches 1.
% As x approaches negative infinity, y approaches 2.

% For y_0 = 1: 
% y approaches 1, as x approaches both positive and negative infinity.
