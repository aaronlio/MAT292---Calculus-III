...
% The time it will integrate until
t1 = pi;
...
% The largest error occurs around t = 1.52.
% Considering the exact solution of the given function, there is an
% inflection point where the function changes concavity. Since ode45 
% approximates the next points based on the previous points, the sudden 
% change of concavity would result in the largest error occuring.
...
% Construct the exact solution
ttt = linspace(1,10,50);
yyy = (3.*ttt-2).^(1/3);

% Plot both on the same figure, plotting the approximation with x's
plot(ttt, yyy, soln.x, soln.y, 'x', 'MarkerSize',10, 'LineWidth', 2);
...
% Plot both on the same figure, plotting the approximation with x's
plot(soln.x, soln.y,'--bs','MarkerSize',6);

% Add a label to the axis and a legend
xlabel('t');
ylabel('y');
legend('Numerical Solution','Location','West');

% Considering the IODE direction field plot of the differential function 
% given, it seems like that there exists an undefined slope (the function 
% is trying to reach infinity) before the function reaches t = 1.
% When ODE45 attempts to approximate a solution of the function, since the
% function increases to infinity rapidly, it is not able to find a small
% enough step size that comes an error that falls within its tolerance
% level. Therefore, it explains that why the warning message states that 
% "[it is] unable to meet integration tolerances without reducing the step 
...
% Defining the variables as symbolic
syms x;

% Defining the function by writing its expression
f = sin(x)*cos(x);

% Manipulating (simplifying) the function
y = simplify(f);

% Computing the values of this function for |x=pi/4| and |x=1|
eval(subs(f,x,pi/4));
eval(subs(f,x,1));
...
% Set up initial approximation phi_0 = 1:
phi_ex8 = sym(1);
 
% Set up a loop to get successive approximations using Picard iterations
n = 5;
for i = 1:n
    func=subs(f_ex8,y,phi_ex8);    % prepare function to integrate: y -> previous phi
    func=subs(func,t,s);      % variable of integration is s, so we change
                              % t -> s  
    phi_ex8 = 1+int(func, s, 0 ,t);   % integrate to find next approximation
end
 
% Plot the approximation
p = ezplot(phi_ex8,[-pi,pi]);
set(p,'Color','green');
hold on
grid on;
 
% The exact solution is |tan(((pi/4)+t)|, plotting it
...
xlabel('t');
ylabel('y');
title('The 5th Picard Approximation and the exact solution to dy/dt = 1+y^2');
legend('Picard phi_5 Approximation', 'Exact Solution','Location','NorthWest');
